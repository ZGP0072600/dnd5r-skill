...
> **Claude Code skill**：让 Claude 成为你的中文 DnD 5e 2024（5r）助手——查证规则、车卡、查法术、**当 DM 带你跑模组**、**为单人 / 双人 / 粉红向 / 克苏鲁风味定制改写模组**，全部基于"DND 五版不全书"中文资料库，**绝不靠训练记忆瞎编**。
...
- **"做个单人粉红向的风骸岛"** → 自动改写模组为定制副本（怪物缩放、NPC 风味调整、剧情骨架不动），跑团时直接用
...
> ⚠️ **模组数据需要你自己准备**：本仓库不附带任何模组内容（版权原因）。你把模组 PDF 放进 `docs/modules/`，第一次开桌时让 Claude 抽取并组织成上述结构即可（已经在 `dnd5r-skill` 工作目录里跑通了"风骸岛之龙"作为示例参考）。
...
- **维度灵活**：不预设"单人 / 粉红"枚举，用户用自然语言描述（"单人 + 克苏鲁 + 主角性转 + 加同伴 NPC"），Claude 解析成结构化改写计划，确认后执行
- **分层改写护栏**（最重要）：
...
**示例**：
```
你：做一个单人粉红向的风骸岛，让米拉狗头人作为协力 NPC 同行
Claude：
  1. 解析需求 → 给你看改写计划表（数值层：怪 HP×0.5；风味层：粉红基调 / 米拉同行）
  2. 你确认 → 复制 docs/modules/风骸岛之龙/ 到 docs/modules/风骸岛之龙__单人粉红向/
...
| 做单人 / 双人 / 粉红向 / 克苏鲁版的 X 模组 | **H 工作流**：解析需求 → 改写计划确认 → 全量副本 → 分层改写 |
