...
> 📦 **DM 跑团 / 模组改写需要你自己准备模组资料**——本仓库**不附带任何模组内容**（版权原因）。你需要把模组 PDF / 文本放进 `docs/modules/<模组名>/`，第一次开桌时让 Claude 抽取并组织成结构化文件（详见下文"DM 跑团"小节）。查证类功能（规则 / 法术 / 怪物 / 车卡）不需要任何准备。
...
> 📦 **前置条件：你需要自备模组资料**——本仓库不附带任何模组内容。把模组 PDF / 文本放进 `docs/modules/<模组名>/`，让 Claude 第一次开桌前抽取并组织。`dnd5r-skill` 工作目录里以"风骸岛之龙"作为流程示例（同样不会上传到这个公开仓库）。

**怎么用**：
1. 把模组 PDF / 资料放在 `docs/modules/<模组中文名>/` 下，由 Claude 抽取并切分成结构化文件：
   - `README.md` — 模组速览 + 剧情骨架 + 🔒 DM 秘密
   - `00_DM跑团指南.md` ~ `0N_第N章_*.md` — 按 PDF 章节切分
   - `NPC速查.md` / `地点速查.md` / `遭遇与战斗.md` / `怪物与法宝速查.md` — 跑团索引
2. 跑团时说"开桌带我跑 X 模组"，Claude 自动：
   - 读模组 README + NPC + 地点
   - 检查 / 帮你车角色卡（按模组推荐起始等级）
   - 在 `.tmp/<战役名>/` 建战役档案（session 日志 + 玩家状态）
   - 用 PDF 原文「读给玩家」段开场，按"场景描述 → 检定 → 推进"4 步循环
3. 内置机制：
   - **🔒 秘密揭示边界**：DM 秘密绝不直接告诉玩家，但会主动提示你可以骰检定揭秘
   - **救场机制**：全队 down 时按模组兜底（如风骸岛 = 茹娜拉营救）
   - **难度自适应**：按当前人数自动取模组里的"二级/三级冒险者"加成
   - **跨 session 恢复**：下次说"继续上次跑团"，读 session-log.md 续接
...
| 你来当 DM 带我跑 X 模组 | **G 工作流**：读模组 → 建战役档案 → 按章节叙事 + 4 步循环跑团 |
| 继续上次跑团 / 上次跑到哪了 | **G7 工作流**：读 `.tmp/<战役>/session-log.md` 续接 |
