...
6. **初始化战役目录**。
   > ⚡ **先跑脚手架点亮面板**（对称 `load-session.py` 的确定性建档）：`python tools/new-campaign.py --data-base .fathom-panels/dnd5r --mode G --module "<模组名>" --name "<战役名>" [--dm-style <风格>]` —— 它确定性地建好 `.fathom-panels/dnd5r/campaigns/<战役名>/campaign.json` 并**复用 load-session.py 写好 session.json**，面板立即亮起（= 一步搞定铁律0 的 ①③）。面板「开桌带团 / 开沙盒」按钮点击时发的就是"请先跑 new-campaign.py"这句话。**沙盒同理**：`--mode I`（不带 `--module`）。脚本**只建 campaign.json + session.json**，下列叙事文件仍由你按本步创建——脚本不碰创作层。

   `campaigns/<战役名>/`（命名建议 `<模组名>-<队伍标识>`，如 `风骸岛之龙-小明组`）完整结构：
   ```
   campaigns/<战役名>/
   ...
   **同步 Panel 快照**（跑了 `new-campaign.py` 就已自动写好、无需手写；仅在推进时更新字段）：Write `.fathom-panels/dnd5r/threads/<threadId>/session.json`（详见 §Panel 数据同步）：`mode="G"`、campaign 整段（名字 / dmStyle 引用名 / inGameTime / location）、players 数组按队伍填、module 初值（name / currentChapter / progress）、combat=null、sandbox=null、lastUpdate。**Fathom 用户切到右侧 panel 看到的就是这份。**
...
#### G2.6. DM 质量控制：叙事、节奏与逻辑

每轮推进前后，执行一次以下检查。它们是通用带团原则，不替代具体战役的 `house-rules.md` 与 `world-state.md`。

**叙事表现**

1. 非战斗场景至少给出可感知的环境信息：空间距离、光线、声音、气味、天气或人物动作；不要只用事件摘要。
2. NPC 必须有独立动机和即时反应，可通过语气、停顿、视线、微表情和动作表现，不把所有潜台词一次性解释完。
3. 队友可以主动观察、提议和反应，但不能替玩家角色说话、接受委托、承诺报酬或作关键决定。
4. 玩家声明行动后，顺序固定为：描述行动过程 → 描述现场反应 → 进行检定/裁定 → 描述结果与后果。
5. 模组没有标注事件的平静场景，不为了制造戏剧性强行添加战斗、威胁、暧昧或阴谋。

**节奏控制**

1. 模组有现成任务钩子时，DM应主动让信件、NPC、传闻或场景把钩子送到玩家面前；不要让队伍无意义地在旅店或据点等待。
2. 主线、据点经营、派系任务和休整要形成可选择的节奏；不要连续塞入多个未经玩家选择的支线。
3. 旅行、调查、交涉和休息可以慢下来；只有发生有意义的选择、检定或状态变化时才推进时间与文件状态。
4. 只有在选择确实重要、信息必须分叉或规则需要确认时才提供选项；平常让玩家直接描述行动。

**逻辑与经济闭合**

1. 每个任务在呈现给玩家前，核对来源、目标、地点、期限、风险、奖励和完成条件；建立任务记录时区分“已发现 / 已收到 / 已接受 / 进行中 / 已完成”。
2. 原模组没有写出的费用、报酬、NPC动机或剧情连接必须标注为“DM扩展”，不能用补写内容冒充原版。
3. 不得设置没有资金来源的硬性经济门槛。若修缮、旅费、装备或经营成本会阻塞主线，必须提供来源明确的收入，或把该成本改为后台推进/可选升级。
4. 不要把声望、情报、据点权限或叙事便利默认折算成金币；除非资料明确写出兑换规则。
5. DM扩展可以补足原版的资金、过渡场景和经营内容，但不能悄悄修改原版任务奖励；扩展报酬应单独记录。
6. 涉及升级、规则数值、生物能力或装备价格时，重新查 `docs/extracted/`，不要用叙事记忆补数值。

**连续性与纠错**

1. 每次重大变化后核对当前时间、地点、等级、HP、资源、NPC关系、已知情报和任务状态；以战役文件与 canonical JSON 为准，不以旧对话印象覆盖文件。
2. 玩家指出逻辑、节奏或资料错误时，先承认并说明影响，再修正原版/DM扩展/废弃内容的边界；不得为维护错误叙事而继续堆叠新解释。
3. 若错误已经影响剧情，给出最小修复：回退到错误前、在当前场景补上，或明确改为DM扩展，并让玩家决定是否需要回退。
4. 通用经验写入本 skill；本战役的特殊修复写入 `campaigns/<战役名>/dm-retro.md` 或 `house-rules.md`，避免污染其他模组。
