...
2. **等玩家声明行动**：不要替玩家决策。
3. **判定**：
   - 模组指定了 DC → 用模组的（写在「遭遇与战斗.md」「地点速查.md」）
...
- 玩家可通过具体检定/对话揭示（DC 已写在 NPC 速查），**主动给玩家机会骰**（"你注意到塔拉克脖子上有刺青，要不要过个智力（历史）？"）。
- 玩家自己脑补对了 → 承认；脑补错了 → 不要主动纠正剧透真相。
- 模组结局未决前不要预告"火花塞计划"这种 DM 视角信息。
